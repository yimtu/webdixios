...
if (cursor && !reduceMotion && matchMedia('(pointer:fine)').matches) {
  ...
  gsap.ticker.add(()=>{x+=(tx-x)*.18;y+=(ty-y)*.18;cursor.style.transform=`translate(${x}px,${y}px) translate(-50%,-50%)`;});
}

if(!reduceMotion){
  const intro=gsap.timeline({defaults:{ease:'power4.out'}});
  intro.from('.hero-kicker',{y:18,opacity:0,duration:.8},.1)
  ...
  gsap.to('.hero-word',{yPercent:22,scale:.92,opacity:.18,ease:'none',scrollTrigger:{trigger:'.hero',start:'top top',end:'bottom top',scrub:true}});
  ...
  document.querySelectorAll<HTMLElement>('[data-cap]').forEach((row,i)=>{
    gsap.from(row.children,{y:32,opacity:0,duration:.85,stagger:.035,ease:'power3.out',scrollTrigger:{trigger:row,start:'top 88%',once:true}});
    const glyph=row.querySelector('.cap-glyph');
    if(glyph)gsap.to(glyph,{x:i%2?-10:10,ease:'none',scrollTrigger:{trigger:row,start:'top bottom',end:'bottom top',scrub:true}});
  });
  ...
  ScrollTrigger.create({trigger:'.process-zone',start:'top top',end:'bottom bottom',scrub:true,onUpdate:self=>{const idx=Math.min(orbitSteps.length-1,Math.floor(self.progress*orbitSteps.length));orbitSteps.forEach((el,i)=>el.classList.toggle('active',i<=idx));if(progress)progress.style.width=`${self.progress*100}%`;}});
  gsap.to('.process-orbit',{rotate:16,ease:'none',scrollTrigger:{trigger:'.process-zone',start:'top top',end:'bottom bottom',scrub:true}});
  gsap.to('.orbit-core',{rotate:-16,ease:'none',scrollTrigger:{trigger:'.process-zone',start:'top top',end:'bottom bottom',scrub:true}});
  ...
  gsap.from('.contact-title, .contact-form',{y:70,opacity:0,duration:1,stagger:.12,ease:'power3.out',scrollTrigger:{trigger:'.contact-grid',start:'top 82%',once:true}});
}
